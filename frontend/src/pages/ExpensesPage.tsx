...
const copy: Record<Lang, any> = {
  en: {
    title: 'Shared expenses', sub: 'One clear place for household spending, your own share, reimbursements, and monthly trends.', add: 'Add expense',
...
export default function ExpensesPage() {
  ...
  return <main className="page shell wide expenses-page expenses-page-v82">
    ...
    <section className="panel expense-history-panel">
      <div className="panel-title-row"><div><p className="eyebrow">ACTIVITY</p><h2>{expenseScope === 'mine' ? c.myExpenses : c.history}</h2>{selectedMonth !== 'all' && <p>{c.selectedMonth}: <strong>{monthLabel(selectedMonth)}</strong> · <button className="text-button" onClick={() => setSelectedMonth('all')}>{c.clearMonth}</button></p>}</div><button className="secondary expense-history-add" onClick={() => { reset(); setOpen(true); }}>＋ {c.add}</button></div>
