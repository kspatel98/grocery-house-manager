...
def ensure_dev_schema(engine: Engine) -> None:
    ...
    statements = [
        ...
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS ocr_provider VARCHAR(50)",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS ocr_status VARCHAR(50) DEFAULT 'manual'",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS ocr_confidence DOUBLE PRECISION",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS currency VARCHAR(12)",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS subtotal_amount DOUBLE PRECISION",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS tax_amount DOUBLE PRECISION",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS discount_amount DOUBLE PRECISION",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS total_amount DOUBLE PRECISION",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS receipt_number VARCHAR(120)",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS payment_method VARCHAR(120)",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS raw_extracted_text TEXT",
        "ALTER TABLE receipts ADD COLUMN IF NOT EXISTS raw_extracted_json TEXT",
        ...
        "CREATE TABLE IF NOT EXISTS receipt_line_items (id SERIAL PRIMARY KEY, receipt_id INTEGER REFERENCES receipts(id) ON DELETE CASCADE, house_id INTEGER REFERENCES houses(id) ON DELETE CASCADE, matched_product_id INTEGER REFERENCES products(id) ON DELETE SET NULL, line_type VARCHAR(50) DEFAULT 'product', description TEXT NOT NULL, normalized_name VARCHAR(220), sku VARCHAR(120), upc VARCHAR(120), quantity DOUBLE PRECISION, line_unit VARCHAR(32), unit_price DOUBLE PRECISION, discount_amount DOUBLE PRECISION, tax_amount DOUBLE PRECISION, line_total DOUBLE PRECISION, confidence DOUBLE PRECISION, needs_review BOOLEAN DEFAULT TRUE, is_selected BOOLEAN DEFAULT TRUE, sort_order INTEGER DEFAULT 0, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW())",
        "CREATE INDEX IF NOT EXISTS ix_receipt_line_items_receipt_id ON receipt_line_items(receipt_id)",
        "CREATE INDEX IF NOT EXISTS ix_receipt_line_items_house_id ON receipt_line_items(house_id)",
        "CREATE INDEX IF NOT EXISTS ix_receipt_line_items_matched_product_id ON receipt_line_items(matched_product_id)",
        "ALTER TABLE receipt_line_items ADD COLUMN IF NOT EXISTS line_unit VARCHAR(32)",
        "ALTER TABLE receipt_line_items ADD COLUMN IF NOT EXISTS inventory_applied BOOLEAN DEFAULT FALSE",
        "ALTER TABLE receipt_line_items ADD COLUMN IF NOT EXISTS inventory_quantity_applied DOUBLE PRECISION",
        "ALTER TABLE receipt_line_items ADD COLUMN IF NOT EXISTS inventory_unit_applied VARCHAR(32)",
        "ALTER TABLE receipt_line_items ADD COLUMN IF NOT EXISTS created_product_from_receipt BOOLEAN DEFAULT FALSE",
        "CREATE INDEX IF NOT EXISTS ix_receipts_ocr_status ON receipts(ocr_status)",
        ...
        "CREATE TABLE IF NOT EXISTS registration_verification_codes (id SERIAL PRIMARY KEY, email VARCHAR(255) NOT NULL, full_name VARCHAR(255) NOT NULL, country VARCHAR(120), city VARCHAR(120), password_hash VARCHAR(255) NOT NULL, code_hash VARCHAR(255) NOT NULL, expires_at TIMESTAMP WITH TIME ZONE NOT NULL, used_at TIMESTAMP WITH TIME ZONE, attempts INTEGER DEFAULT 0, existing_user_id INTEGER REFERENCES users(id) ON DELETE CASCADE, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW())",
        "CREATE INDEX IF NOT EXISTS ix_registration_verification_codes_email ON registration_verification_codes(email)",
        "CREATE INDEX IF NOT EXISTS ix_registration_verification_codes_expires_at ON registration_verification_codes(expires_at)",
        "CREATE INDEX IF NOT EXISTS ix_registration_verification_codes_created_at ON registration_verification_codes(created_at)",
